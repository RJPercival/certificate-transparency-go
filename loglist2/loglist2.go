...
func NewFromJSON(llData string) (*LogList, error) {
	var ll LogList
	if err := jsonpb.UnmarshalString(llData, &ll); err != nil {
...
// raw signature data.
func NewFromSignedJSON(llData string, rawSig []byte, pubKey crypto.PublicKey) (*LogList, error) {
	...
	if err := tls.VerifySignature(pubKey, []byte(llData), tlsSig); err != nil {
